...
$textFile=$_SERVER["DOCUMENT_ROOT"].$dirPath.$aurtraCompanyID.'/loggerNo'.$aurtraID.'/AnalysisSummary'.$aurtraID.".pdf";
$urlFile=$dirPath.$aurtraCompanyID.'/loggerNo'.$aurtraID.'/AnalysisSummary'.$aurtraID.".pdf";
$showFile="no";
if(file_exists($textFile)) $showFile="yes";

?>  

<?php
	$ourCurrentUser = wp_get_current_user();
    if ((count($ourCurrentUser->roles)==1) AND (($ourCurrentUser->roles[0]=='company_user_role') OR ($ourCurrentUser->roles[0]=='company_admin_role'))){
?>	
<div style='margin: 20px 0'><h1><?php echo get_the_title(); ?> </h1></div>
<div class="row">
<div class="col-lg-2 dataLabel">Asset ID</div><div class="col-lg-10 dataLabel"><?php echo esc_html__( $companyPost->post_title)." Asset:  ".esc_html__(get_post_field('deviceID'));?>
<span class="deviceTxt_span">

<?php if($showFile=="yes"){?>
<input name="showFile" type="button" id="showFile" class="button deviceTxt_btn" value="Analysis Summary and TILAS Life Assessment" onclick="showFile('<?php echo $urlFile?>');" />
<?php } ?>
...
<div class="row"><div class="col-lg-12"><input name="return" type="button" id="returntoassets" class="button" value="Return to assets" />
...
<input name="return" type="button" id="returntoassets" class="button" value="Return to assets" />
 
  
<input name="cancelupdate" type="button" id="cancelupdate" class="button" value="Cancel" />                 
