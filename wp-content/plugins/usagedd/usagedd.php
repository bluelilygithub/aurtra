...
/*
You can change the CSS for the usage display. DO NOT USE
SINGLE-QUOTE ' CHARACTERS IN YOUR CSS!! You have been warned.
*/
...
');
// CSS ends before the previous line
...
class UsageDD {
	...
	function time_to_first_byte() {
		...
		if (!empty($_SERVER['REQUEST_TIME_FLOAT'])) {
			...
			$this->servertime = strval(round(microtime(true) - $this->starttime, 2)) . 's&nbsp;|&nbsp;';
		}
	}
	...
	function display_usage() {
		if ($this->display && !defined('WP_INSTALLING') && (!defined('DOING_AJAX') || (defined('DOING_AJAX') && $this->ajax)) && current_user_can('update_core')) {
			...
			echo ((defined('DOING_AJAX')) ? '' : $this->css . '</style><div id="usage_dd_spacer"></div>') . '<div class="usage-dd"' . ((defined('DOING_AJAX') && $this->ajax) ? '>' : ' id="usage_dd">') . "{$wpdb->num_queries}Q<span>{$time_usage}s</span>{$memory_usage}M</div>";
		}
	}
}

// allow ToolkitDD to control UsageDD configurable options
