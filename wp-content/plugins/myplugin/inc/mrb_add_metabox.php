<?php
function mrb_add_custom_metabox() {
	add_meta_box(
		'mrb_meta',
		__( 'Job Listing' ),
		'mrb_meta_callback',
...
add_action( 'add_meta_boxes', 'mrb_add_custom_metabox' );
function mrb_meta_callback( $post ) {
	wp_nonce_field( basename( __FILE__ ), 'mrb_jobs_nonce' );
	$mrb_stored_meta = get_post_meta( $post->ID ); ?>
	...
	<div>
		<div class="meta-row">
			...
			<div class="meta-td">
				...
				value="<?php if ( ! empty ( $mrb_stored_meta['job_id'] ) ) {
					echo esc_attr( $mrb_stored_meta['job_id'][0] );
				} ?>"/>
			</div>
		</div>
		<div class="meta-row">
			...
			<div class="meta-td">
				<input type="text" size=10 class="dwwp-row-content datepicker" name="date_listed" id="date-listed" value="<?php if ( ! empty ( $mrb_stored_meta['date_listed'] ) ) echo esc_attr( $mrb_stored_meta['date_listed'][0] ); ?>"/>
			</div>
		</div>
		<div class="meta-row">
			<div class="meta-th">
			...
			<div class="meta-td">
				<input type="text" size=10 class="dwwp-row-content datepicker" name="application_deadline" id="application_deadline" value="<?php if ( ! empty ( $mrb_stored_meta['application_deadline'] ) ) echo esc_attr( $mrb_stored_meta['application_deadline'][0] ); ?>"/>
			</div>
		</div>
		<div class="meta">
			<div class="meta-th">
		...
		<div class="meta-row">
	        ...
	        <div class="meta-td">
	          ...
	          if ( ! empty ( $mrb_stored_meta['minimum_requirements'] ) ) {
		          echo esc_attr( $mrb_stored_meta['minimum_requirements'][0] );
	          }
	          ?>
	          </textarea>
	        </div>
	    ...
	    <div class="meta-row">
	        ...
	        <div class="meta-td">
	          <textarea name="preferred_requirements" class="dwwp-textarea" id="preferred-requirements"><?php
			          if ( ! empty ( $mrb_stored_meta['preferred_requirements'] ) ) {
			            echo esc_attr( $mrb_stored_meta['preferred_requirements'][0] );
			          }
		          ?>
	          </textarea>
	        </div>
	    ...
	    <div class="meta-row">
	        ...
	        <div class="meta-td">
	          <select name="relocation_assistance" id="relocation-assistance">
		          <option value="Yes" <?php if ( ! empty ( $mrb_stored_meta['relocation_assistance'] ) ) selected( $mrb_stored_meta['relocation_assistance'][0], 'Yes' ); ?>><?php _e( 'Yes', 'wp-job-listing' )?></option>';
		          <option value="No" <?php if ( ! empty ( $mrb_stored_meta['relocation_assistance'] ) ) selected( $mrb_stored_meta['relocation_assistance'][0], 'No' ); ?>><?php _e( 'No', 'wp-job-listing' )?></option>';
	          </select>
	    </div> 
	</div>	
	<?php
}
function mrb_meta_save( $post_id ) {
	// Checks save status
    $is_autosave = wp_is_post_autosave( $post_id );
    $is_revision = wp_is_post_revision( $post_id );
    $is_valid_nonce = ( isset( $_POST[ 'mrb_jobs_nonce' ] ) && wp_verify_nonce( $_POST[ 'mrb_jobs_nonce' ], basename( __FILE__ ) ) ) ? 'true' : 'false';
...
add_action( 'save_post', 'mrb_meta_save' );